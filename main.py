import ctypes
ctypes.windll.kernel32.SetConsoleTitleW("Helios")
...
def buy(json, itemid, productid, prox, itemName, itemQuan, itemSerial, itemID):
    ...
    while 1:
        try:
            ...
            if not bought["purchased"]:
                ...
                embed = DiscordEmbed(title='Sniped UGC', description='You sniped a UGC, information is below.', color='03b2f8')
...
while 1:
    ...
    for limited in limiteds:
        ...
        if info.get("priceStatus", "") != "Off Sale" and info.get("collectibleItemId") is not None:
            ...
            buy(info, info["collectibleItemId"], productid, proxy, info["name"], info["totalQuantity"], info["totalQuantity"] - info["unitsAvailableForConsumption"], info["id"])
